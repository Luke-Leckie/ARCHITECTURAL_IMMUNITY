...
def assign_chambers_2 (D1NE, MEAN_TUN_DIAMS, ch_tun_thrsh, edge_list, img_stack, NE_threshold, slope_max_thresh, slope_max_thresh2, intensity_thresh, allowance, scale_factor, increment, decrement, x_cm=None, y_cm=None, z_cm=None, voxel=None):
    if not isinstance(D1NE, bool):
        raise TypeError("D1NE must be a boolean value (True or False) if you want nest entrances to only be D1")
    if voxel:
        x_scale = y_scale = z_scale = voxel
    ...
    for node in internal_nodes:
        ...
        if mean_slope<slope_max_thresh and intensity_diff>intensity_thresh:
            ...
            chambers.append(node)
            print('chamber=', node)
        elif  (mean_diam*x_scale)>(MEAN_TUN_DIAMS*ch_tun_thrsh) and mean_slope<slope_max_thresh2:
            ellipsoid_vol= 1.33 * math.pi * x_rad * y_rad * z_rad
            ellipsoid_vols.append(ellipsoid_vol)
            chambers.append(node)
            print('chamber=', node)
