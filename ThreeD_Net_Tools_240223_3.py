    ...
    for px2 in range(y_px, y_upper_bound):
        ...
        if px2 in range(0,len(img_stack[0][0])):
            ...
            if y_point>0:
                ...
                y_slope=y_difference/px2_distance
...
def assign_chambers_2 (D1NE, MEAN_TUN_DIAMS, ch_tun_thrsh, edge_list, img_stack, NE_threshold, slope_max_thresh, intensity_thresh, allowance, scale_factor, increment, decrement, x_cm=None, y_cm=None, z_cm=None, voxel=None):
    if not isinstance(D1NE, bool):
        raise TypeError("D1NE must be a boolean value (True or False) if you want nest entrances to only be D1")
    if voxel:
        x_scale = y_scale = z_scale = voxel
    ...
    for node in internal_nodes:
        ...
        if x_mean_slope_cm==0 and y_mean_slope_cm==0:
            ...
            mean_slope=(x_mean_slope_cm+y_mean_slope_cm)/2
        print('intensity diff=', intensity_diff)
        if mean_slope<slope_max_thresh and intensity_diff>intensity_thresh or (mean_diam*x_scale)>(MEAN_TUN_DIAMS*ch_tun_thrsh) and mean_slope<slope_max_thresh:
            ellipsoid_vol= 1.33 * math.pi * x_rad * y_rad * z_rad
            ellipsoid_vols.append(ellipsoid_vol)
            chambers.append(node)
            print('chamber=', node)
...
def calculate_diameter(G):
    ...
    for node1, path_dict in shortest_paths:
        for node2, path_length in path_dict.items():
            if node1 != node2:
                if path_length > max_diameter:
                    max_diameter = path_length
                    edges_spatial = [(node1, node2)]
                elif path_length == max_diameter:
                    edges_spatial.append((node1, node2))
    ...
    for node1, node2 in itertools.combinations(G.nodes, 2):
        ...
        if path_length > max_edge_diameter:
            ...
            edges_edge = [path]
        elif path_length == max_edge_diameter:
            edges_edge.append(path)
    
    return max_diameter, max_edge_diameter, max_edge_path, shortest_paths, path
